...
async def create_overlay(payload: ImageOverlayRequest):
    """
    Ajoute un overlay professionnel sur une image
    
    Prend une image de base (URL) et ajoute :
    - Un titre
    - 3 bullet points
    
    Style LinkedIn professionnel avec bande sombre en bas.
    """
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erreur génération overlay: {str(e)}"
        )
...
async def download_image(filename: str):
    ...
    Retourne l'image en tant que fichier JPEG
    """
    ...
    # Vérifier qu'on ne remonte pas dans l'arborescence (sécurité)
