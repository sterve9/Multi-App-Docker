...
class Video(Base):
    ...
    script = Column(JSON, nullable=True)  # Liste de scènes [{scene, narration, image_prompt}]
    tags = Column(JSON, nullable=True)

    # Fichiers générés
    scenes_images = Column(JSON, nullable=True)  # URLs images par scène
    scenes_audio = Column(JSON, nullable=True)   # Chemins audio par scène
